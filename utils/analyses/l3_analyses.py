...
L3_ATTACK_LOGS = Path(__file__).resolve().parents[2] / "logs/l3attack.jsonl"
BEST_THRDS_NUM_RESULTS = Path(__file__).resolve().parents[0] / "results/l3/best_thrds_num.jsonl"


def search_best_thrds_num():
    logs = load_jsonl(filename=L3_ATTACK_LOGS)
    data = {}
    threads_arr = []
